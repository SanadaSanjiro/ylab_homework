...
import website.ylab.financetracker.in.servlet.auth.AddUserServlet;
...
class AddUserServletTest {
    ...
    void doPost() {
        ...
        Mockito.when(userService.addNewUser(Mockito.any())).thenReturn(
        ...
        try (MockedStatic<ServiceProvider> mock = Mockito.mockStatic(ServiceProvider.class)) {
            ...
            Mockito.verify(userService, Mockito.times(1)).addNewUser(Mockito.any());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
