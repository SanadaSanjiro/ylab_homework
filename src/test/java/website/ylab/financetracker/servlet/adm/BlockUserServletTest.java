...
import website.ylab.financetracker.in.servlet.adm.BlockUserServlet;
import website.ylab.financetracker.service.ServiceProvider;
import website.ylab.financetracker.service.auth.UserService;

class BlockUserServletTest {
    long id =1;

    @Test
    void doPut() {
        UserService service = Mockito.mock(UserService.class);
        Mockito.when(service.blockUser(Mockito.anyLong())).thenReturn(
        ...
        try (MockedStatic<ServiceProvider> mock = Mockito.mockStatic(ServiceProvider.class)) {
            ...
            new BlockUserServlet().doPut(req, resp);
            Mockito.verify(req, Mockito.atLeast(1)).getParameter("id");
            Mockito.verify(service, Mockito.times(1))
                    .blockUser(Mockito.anyLong());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
