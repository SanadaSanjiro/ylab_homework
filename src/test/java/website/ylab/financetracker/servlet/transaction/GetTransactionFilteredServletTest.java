...
import website.ylab.financetracker.in.servlet.transaction.GetTransactionFilteredServlet;
...
class GetTransactionFilteredServletTest {
    long id = 1L;
    ...
    void doGet() {
        ...
        try (MockedStatic<ServiceProvider> mock = Mockito.mockStatic(ServiceProvider.class)) {
            ...
            Mockito.when(service.getFiltered(Mockito.any())).thenReturn(List.of(getResponse()));
            ...
            Mockito.when(req.getParameter("type")).thenReturn(type);
            Mockito.when(req.getParameter( "category")).thenReturn(category);
            Mockito.when(req.getParameter("date")).thenReturn(date);
            Mockito.when(resp.getOutputStream()).thenReturn(out);
            new GetTransactionFilteredServlet().doGet(req, resp);
            Mockito.verify(req, Mockito.atLeast(1)).getParameter("userid");
            Mockito.verify(service, Mockito.times(1)).getFiltered(Mockito.any());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
