...
class AddTransactionServletTest {
    long id = 1L;
    ...
    void doPost() {
        ...
        try (MockedStatic<ServiceProvider> mock = Mockito.mockStatic(ServiceProvider.class)) {
            ...
            Mockito.when(service.addNewTransaction(Mockito.any())).thenReturn(getResponse());
            ...
            new AddTransactionServlet().doPost(req, resp);
            Mockito.verify(req, Mockito.atLeast(1)).getParameter("amount");
            Mockito.verify(service, Mockito.times(1)).addNewTransaction(Mockito.any());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
