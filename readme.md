...
Для выполнения всех операций, кроме регистрации нового пользователя, 
требуется предварительно выполнить вход в систему в разделе /auth/login, передав параметры
...
### Логирование и аудит
Выполняется логирование времени исполнения всех контроллеров, помеченных аннотацией @Loggable.
Также, выполняется аудит действий администраторов с данными пользователей. Контроллеры, по которым ведется аудит,
помечены аннотацией @Auditable.
...
POST{адрес сайта}/user/add - инициирует регистрацию нового пользователя. По умолчанию все пользователи 
...
```
POST{адрес сайта}/auth/logout - отключает от системы текущего пользователя.

GET{адрес сайта}/auth/check - возвращает JSON с информацией о текущей сессии пользователя 
...
GET{адрес сайта}/user/get - возвращает данные учетной записи пользователя, за исключением пароля.


DELETE{адрес сайта}/user/delete - удаляет профиль пользователя и все его данные из системы.
...
DELETE{адрес сайта}/transaction/delete/{номер транзакции} - удаление транзакции, введенной ранее. Номер удаляемой 
транзакции указывается в переменной пути.

GET{адрес сайта}/transaction/get/{номер транзакции} - поиск транзакции по ее id. Номер 
транзакции указывается в переменной пути.

POST{адрес сайта}/transaction/filtered Возвращение списка всех транзакций пользователя с возможностью фильтрации по дате, 
...
GET{адрес сайта}/budget/get - отслеживание превышения бюджета пользователя.
...
```
GET{адрес сайта}/target/get - отслеживание прогресса по цели.

#### Статистика и аналитика. 
Предоставляет следующие операции:
GET{адрес сайта}/stat/balance - подсчёт текущего баланса.

POST{адрес сайта}/stat/turnover - расчёт суммарного дохода и расхода за определённый период. Формат тела запроса:
```
{
    "startDate": "04-03-2025"
}
```
GET{адрес сайта}/stat/expenses - анализ расходов по категориям.
...
GET{адрес сайта}/api/exceedance - API для напоминаний о достижении лимитов расходов. 
Возвращает true, если лимит бюджета данного пользователя был превышен.

GET{адрес сайта}/api/notifications - возможность интеграции с email-уведомлениями. 
...
GET{адрес сайта}/adm/users - возвращает список пользователей системы.

GET{адрес сайта}/adm/transactions/{id пользователя} - отображает список транзакций указанного пользователя.
...
где id - номер пользователя
```
DELETE{адрес сайта}/adm/delete/{id пользователя} - удаление пользователя. Полностью удаляет учетную запись 
и все пользовательские данные.
Формат тела запроса:
